...
def scan_period_signals(stock_dict, days_lookback, progress_bar, min_vol, bias_thresh, strategy_type, 
    ...
    for i, batch_idx in enumerate(range(0, len(all_tickers), BATCH_SIZE)):
        ...
        progress_bar.progress(prog, text=f"正在編制戰情報告 (同步邏輯)...({int(prog*100)}%)")
...
with st.sidebar:
    ...
    with st.expander("📅 系統開發日誌 (Release Notes)"):
        ...
        **發布日期**: 2025-01-03
        ...
        #### **[CORE] 核心架構升級**
        * **Multi-View Architecture**: 導入多視圖並存架構，解決了「今日篩選」、「週報戰情」與「歷史回測」介面互斥的問題，實現真正的儀表板 (Dashboard) 體驗。
        * **Logic Synchronization**: 徹底重構了回測引擎，確保「週報回測」與「今日即時篩選」使用完全一致的演算法與參數 (含 KD、趨勢、量能過濾)，消除數據偏差。
        * **State Persistence**: 強化 Session State 管理，確保在切換功能時，已運算的數據不會丟失，提升操作流暢度。

        #### **[UI/UX] 介面與體驗優化**
        * **Stacked Report**: 將原本分散的週報數據整合為「戰情堆疊表」，依日期排序，讓使用者能一目了然過去 5 日的市場動態。
        * **Dynamic Metrics**: 修正了回測報告中的關鍵指標 (總結算、勝率、損益)，現在會依據篩選結果即時動態計算，提供精準的策略成效評估。
        * **Security Layer**: 實裝存取控制層 (ACL)，透過密碼鎖定機制保護系統資源，並整合社群導流功能。

        #### **[ALGO] 演算法與效能**
        * **High-Concurrency Fetching**: 恢復多線程 (Multi-thread) 數據抓取機制，並優化 Batch Size 至 50，顯著提升大盤掃描速度。
        * **Resilience Handling**: 增加對 Yahoo Finance API 的例外處理與重試機制，降低因連線逾時導致的數據缺漏。
        """)
...
if st.session_state['backtest_result'] is not None:
    ...
    if not df_watching.empty:
        ...
        <div style="background-color: #fff8dc; padding: 15px; border-radius: 10px; border: 2px solid #ffa500; margin-bottom: 20px;">
            <h3 style="color: #d2691e; margin:0;">👀 旺來關注中 (近期訊號)</h3>
            <p style="color: #666; margin:5px 0 0 0;">這些是最近 10 天內出現訊號，且尚未結算的股票 (等同週報清單)。</p>
        </div>
        """, unsafe_allow_html=True)
