...
with st.sidebar:
    ...
    if bias_threshold <= 5.0:
        st.caption(f"🛡️ **防守型**：設定在 {bias_threshold}% 以內，適合買在起漲點或回測支撐。(皆有波段機會)")
    else:
        st.caption(f"⚔️ **攻擊型**：範圍擴大至 {bias_threshold}%，鎖定強勢噴出股。(皆有波段機會)")
        st.warning("⚠️ **注意**：攻擊型若遇到「生命線下彎幅度過大」，容易形成假突破，請小心風險！")
